...
STRATEGIES = [
    ...
    OptimizationStrategy(
        ...
        strategy_type=StrategyType.LOOP_OPTIMIZATION,
        description="Move loop-invariant computations outside the loop. "
                    "Expressions that don't change across iterations are computed once.",
        applicable_patterns=["loop with constant expression", "invariant computation", "loop optimization"],
        expected_speedup="1.5x-3x",
        risk_level="LOW",
        examples=[
            "for i: x = a * b + arr[i]  ->  tmp = a * b; for i: x = tmp + arr[i]",
            "Loop-invariant multiplication hoisted before loop",
        ]
    ),
    OptimizationStrategy(
        name="Modulo-to-Bitwise",
        strategy_type=StrategyType.STRENGTH_REDUCTION,
        description="Replace modulo by powers of 2 with bitwise AND. "
                    "x % 2^n becomes x & (2^n - 1) for unsigned values.",
        applicable_patterns=["modulo by constant", "power of 2 modulo", "remainder"],
        expected_speedup="3x-5x",
        risk_level="MEDIUM",
        examples=[
            "urem i32 %x, 8  ->  and i32 %x, 7",
            "urem i32 %x, 16 ->  and i32 %x, 15",
        ]
    ),
    OptimizationStrategy(
        name="Multiply-Add-Fusion",
        strategy_type=StrategyType.STRENGTH_REDUCTION,
        description="Fuse multiply-add sequences into FMA (Fused Multiply-Add) operations. "
                    "Reduces latency and improves precision for floating-point.",
        applicable_patterns=["multiply then add", "dot product", "polynomial evaluation", "FMA"],
        expected_speedup="1.3x-2x",
        risk_level="LOW",
        examples=[
            "a * b + c  ->  fma(a, b, c)",
            "Multiply-add chains in dot products",
        ]
    ),
    OptimizationStrategy(
        name="Branch-to-Select",
        strategy_type=StrategyType.CONDITIONAL,
        description="Convert simple conditional branches to select (conditional move) instructions. "
                    "Eliminates branch misprediction overhead for simple conditions.",
        applicable_patterns=["simple if-else", "conditional assignment", "min/max pattern", "abs pattern"],
        expected_speedup="1.5x-3x",
        risk_level="MEDIUM",
        examples=[
            "if (x > y) z = x; else z = y;  ->  z = select(x > y, x, y)",
            "Conditional move for simple value selection",
        ]
    ),
    OptimizationStrategy(
        name="Range-Check-Elimination",
        strategy_type=StrategyType.DEAD_CODE_ELIMINATION,
        description="Remove redundant bounds checks when the range is already proven safe. "
                    "Useful for loop indices with known bounds.",
        applicable_patterns=["array bounds check", "redundant comparison", "loop index check"],
        expected_speedup="1.2x-1.5x",
        risk_level="HIGH",
        examples=[
            "Loop with i < n already ensures arr[i] is in bounds",
            "Remove duplicate range checks in tight loops",
        ]
    ),
]
