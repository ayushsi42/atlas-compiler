"""
The @jit Decorator - Entry point for Atlas.

This decorator intercepts Python functions and runs them through 
the full Atlas pipeline: Lift -> Optimize -> Verify -> Compile -> Execute.
"""
...
from atlas.config import get_config, AOTGPTConfig
from atlas.lifter import lift_function, LiftedFunction, IRCache
from atlas.neural import NeuralOptimizer
from atlas.verifier import prove_equivalence
from atlas.executor import compile_and_execute
...
class JITCompiler:
    ...
    def compile(
        ...
        """
        Compile a function through the Atlas pipeline.
        ...
        try:
            ...
            if self.config.verbose:
                print(f"[Atlas] CEGAR failed: {e}, using original")
...
def jit(
    ...
    *,
    ...
    """
    The @atlas.jit decorator.
    
    Compiles Python functions through the Atlas pipeline:
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if compiled_fn is None:
                ...
                try:
                    ...
                    if verbose:
                        print(f"[Atlas] Compilation failed, using original: {e}")
                    compiled_fn = fn
        ...
        # Attach metadata
        wrapper._atlas_original = fn
        wrapper._atlas_stats = lambda: stats
...
def get_stats(func: Callable) -> Optional[JITStats]:
    ...
    stats_fn = getattr(func, '_atlas_stats', None)
    if stats_fn:
        return stats_fn()
    return None
...
def get_original(func: Callable) -> Optional[Callable]:
    ...
    return getattr(func, '_atlas_original', None)
