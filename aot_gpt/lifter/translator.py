...
def _infer_signature_str(func: Callable, sample_args: Optional[Tuple] = None) -> str:
    ...
    For MVP, we assume int64 for all arguments if not specified.
    """
    sig = inspect.signature(func)
    num_params = len(sig.parameters)
    
    if sample_args:
        # Infer from sample arguments
        type_strs = []
        for arg in sample_args:
            if isinstance(arg, int):
                type_strs.append("int64")
            elif isinstance(arg, float):
                type_strs.append("float64")
            else:
                type_strs.append("int64")  # fallback
    else:
        # Default to int64 for all params (MVP restriction)
        type_strs = ["int64"] * num_params
    
    # Return type is int64 for MVP
    return f"int64({', '.join(type_strs)})"
...
def lift_function(
    ...
    try:
        ...
        # Infer types for metadata
        num_params = len(inspect.signature(func).parameters)
        if sample_args:
            arg_types_tuple = tuple(
                numba_types.int64 if isinstance(arg, int)
                else numba_types.float64 if isinstance(arg, float)
                else numba_types.int64
                for arg in sample_args
            )
        else:
